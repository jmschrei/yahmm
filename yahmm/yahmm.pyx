...
(4, State(B-end, None))])
	...
	def _train_baum_welch(self, sequences, stop_threshold, min_iterations, 
		...
		while improvement > stop_threshold or iteration < min_iterations:
			...
			# If an improvement measure is wanted, then calculate the sum of
			# the log probabilities before and after training.
			log_probability_sum = self.log_probability( sequences )
	...
	cdef double _train_once_baum_welch(self, numpy.ndarray sequences, 
		...
		cdef list weights = [ [] for i in xrange( self.silent_start ) ]
		...
		for sequence in sequences:
			...
			for k in xrange( m ):
				...
				if k < self.silent_start:
					# Now think about the emission probabilities for each state.
					# We want to have a weighting of each observation, based on
					# the probability that it was generated by the state in
					# question. However, we also want to tie together
					# observations based on tied states, if they exist. So we
					# will only go through each set of tied states once, to not
					# update the underlying distribution too many times.

					# If we've already visited this state, stop.
					if visited[k] == 1:
						continue
					visited[k] = 1

					# Mark that we've visited all other states in the tied state
					# set now, so we don't revisit them and waste time.
					...
					for i in xrange( n ):
						...
						# docs/HTKBook/node7_mn.html, we really should divide by
						# sequence probability.
						tied_probability = cexp( f[i+1, k] + b[i+1, k] - 
							log_sequence_probability )

						# Now add the probability that this observation was 
						# generated in any of the other states which are tied
						# to this state. 
						for l in xrange( tied_states[k], tied_states[k+1] ):
							li = self.tied[l]
							tied_probability += cexp( f[i+1, li] + 
								b[i+1, li] - log_sequence_probability )

						# If the probability is above a threshold, then add
						# the symbol and its weight to all states in the tied
						# state set.
						print k, self.states[k].name, sequence[i], tied_probability
						if tied_probability < emitted_probability_threshold:
							continue

						# Add the symbol and weight to this state. Since this
						# is the only state we will be visiting later, we don't
						# need to update the other states in the set of tied 
						# states, to save space.
						symbols[k].append( sequence[i] )
						weights[k].append( tied_probability )
		...
		# Set visited to be 0's again.
		visited = numpy.zeros( self.silent_start,
			dtype=numpy.int32 )
